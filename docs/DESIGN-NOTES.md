...
- **CF-1 (dep hygiene): libSQL bundles C SQLite.** VERIFIED `libsql-ffi-0.9.30/bundled/src/sqlite3.c` (8.9 MB) compiled by its build.rs — `core` feature pulls it; there is no pure-Rust path. Violates locked decisions 1 & 3 and the design's own CI gate. Fix: `db/schema.sql` is now the canonical *logical* schema; the execution backend is reopened (OI-1) for an operator ruling. Added CI gate `! cargo tree | grep libsql-ffi`. (Wording in R9, THREAT-MODEL A4/FS-S2 updated.)
...
### R9 — Store backend C-dep tension (REOPENED by the review — see OI-1)
The synthesized design claimed "libSQL pure-Rust path." This is FALSE (CF-1, VERIFIED). `db/schema.sql` is retained as the canonical logical model; the execution backend is an OPEN ITEM requiring an operator ruling. The `inmem-store` feature (RAM-only) remains the CI test analogue regardless of backend.
...
- **The chosen at-rest store** (pending OI-1) is heavier than the few-deps tenet prefers but is the locked storage pillar; mitigated by `default-features` minimization + the `inmem-store` test feature + the no-C CI gate.
...
|---|---|---|---|
| **OI-1** | CRITICAL/blocking | **At-rest store backend.** libSQL bundles C SQLite (VERIFIED), violating locked decisions 1/3. Needs an operator ruling: (a) adopt a pure-Rust embedded store (e.g. `redb`) expressing `db/schema.sql` as keyspaces — honors no-C, loses SQL ergonomics; (b) explicitly WAIVE the no-C tenet for libSQL — keeps SQL, breaks the tenet & CI gate; (c) `rusqlite` (still C). Recommendation: (a). Until ruled, `db/schema.sql` is the canonical logical model and code targets a `Store` trait with an `inmem-store` impl. | Awaiting operator decision; flagged as the top blocker. |
