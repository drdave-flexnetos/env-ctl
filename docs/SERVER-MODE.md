...
- **Recommended on-box store wiring:** an **embedded `sqld` bound to loopback only (no exposed Hrana/gRPC port to the network)**, and `secretd` talks to it via the **pure-Rust `remote` libSQL client** (`default-features=false, features=["remote"]` — VERIFIED pure-Rust, no `libsql-ffi`). The C core then lives in the separate `sqld` process, isolated from `secretd`'s key-handling and the network edge. The relay edge speaks the typed engine API; the engine owns all (parameterized) SQL; attacker bytes never reach the SQL parser.
...
- STORE crate scoped waiver: `cargo tree -p envctl-secrets-store-libsql --no-default-features --features remote` MUST show **no** `libsql-ffi` (pure-Rust remote client = the recommended wiring); the embedded `core` build is the documented, bounded waiver.
...
4. **`decide()` adds exactly ONE clause (default-deny preserved):** for a remote bearer, REQUIRE `req.remote == Some` AND `req.remote.client_id == bearer.client_id` AND the DPoP thumbprint matches the registered `jkt` AND the per-request DPoP proof verified at the edge — else `Deny(PeerMismatch | RemoteClientUnknown)`. **Cross-kind never crosses:** a remote bearer presented over loopback (`req.remote == None`), or a local bearer presented remotely, is DENIED.
